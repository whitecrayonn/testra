...
> **Sprint 4 status (2026-08-05):** Since Sprint 3, the following controls were added or hardened (see `SPRINT_BACKLOG.md` M1 status note and PR #15 for detail): account lockout after repeated failed logins with uniform-response account-enumeration resistance (identity, item below); refresh-token invalidation and login-lockout clearing on password reset; a magic-link (HTTPS URL, not a bare token) password-reset email; SSRF protection now re-validates and pins the outbound connection to the exact resolved address at dial time (`security.SafeDialContext`/`SafeHTTPClient`, wired into every `ValidateURL` caller), so the check and the actual connection can no longer diverge via DNS rebinding — the earlier `ValidateURL` pre-flight check is uncached and is only a fast-fail UX convenience, not the security boundary; a CSP `report-uri` endpoint with body-size/shape limits and layered (per-client + global) rate limiting; and a first pass at CI dependency scanning (Trivy + SBOM, currently report-only pending remediation of pre-existing CVEs it surfaced — see the follow-up task). This does not itself constitute the sign-off below; a reviewer still needs to check the relevant boxes with evidence before launch.
