...
func (r *SQLRepository) Create(ctx context.Context, workspace *Workspace) error {
	_, err := r.db.ExecContext(ctx,
		...
		workspace.ID, workspace.OrganizationID, workspace.Name, workspace.Slug, workspace.Description, workspace.CreatedAt, workspace.UpdatedAt,
	)
	return err
}
...
func (r *SQLRepository) GetByID(ctx context.Context, id uuid.UUID) (*Workspace, error) {
	...
	err := r.db.QueryRowContext(ctx,
		`SELECT id, organization_id, name, slug, description, created_at, updated_at FROM workspaces WHERE id = $1`,
		id,
	).Scan(&w.ID, &w.OrganizationID, &w.Name, &w.Slug, &w.Description, &w.CreatedAt, &w.UpdatedAt)
...
func (r *SQLRepository) GetBySlug(ctx context.Context, orgID uuid.UUID, slug string) (*Workspace, error) {
	var w Workspace
	err := r.db.QueryRowContext(ctx,
		`SELECT id, organization_id, name, slug, description, created_at, updated_at FROM workspaces
...
func (r *SQLRepository) ListForOrganizationPaginated(ctx context.Context, orgID uuid.UUID, cursor string, limit int) ([]Workspace, error) {
	if cursor != "" {
		...
		for rows.Next() {
			var w Workspace
			if err := rows.Scan(&w.ID, &w.OrganizationID, &w.Name, &w.Slug, &w.Description, &w.CreatedAt, &w.UpdatedAt); err != nil {
				return nil, err
			}
