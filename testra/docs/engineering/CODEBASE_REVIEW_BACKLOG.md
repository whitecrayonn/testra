...
|----|------|-------|-------|-----------|--------|--------------|--------|
| **PM-01** | Product Maturity | Tenant onboarding & self-service billing | `apps/api/internal/billing/*`, `apps/web/app/(dashboard)/settings/billing/*` | No billing module is wired end-to-end; SaaS launches cannot collect revenue or enforce plan limits. | L | Stripe integration, plan seed data | Blocks commercial launch |
...
| **OBS-02** | Observability | Define and instrument SLO/SLI dashboards (latency, error budget, queue depth) | `docs/operations/MONITORING_LOGGING_GUIDE.md`, `scripts/ops/observability/*` (not yet written) | No alertable SLOs or runbooks; incidents will be detected late. | M | OBS-01, Grafana/Loki stack | High |
...
| **SEC-02** | Security Posture | Implement a secrets-manager backend (e.g., Vault) as an optional `secrets.Provider` implementation | `apps/api/internal/shared/config/config.go`, `apps/api/internal/shared/secrets/*` | `SecretProvider` is a thin env wrapper; no VM environment file or deployment config exists yet to harden. | L | VM provisioning (SBL-025–SBL-028) | Critical |
...
| **PROD-01** | Production Readiness | Write VM provisioning scripts and process-supervisor/reverse-proxy config | `scripts/ops/*` (not yet written; see `docs/deployment/DEPLOYMENT_GUIDE.md`) | Nothing exists yet — no provisioning script, service units, or reverse-proxy config. | L | Rented VM, domain | Critical |
...
| Production Readiness | 3 | No VM provisioning scripts or deployment config exist yet; no tested DR or secrets manager. |
